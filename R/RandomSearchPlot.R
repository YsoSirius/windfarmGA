...
RandomSearchPlot <- function(resultRS, result, Polygon1, best) {
  ...
  resBest <- dplyr::arrange(resBest, EnergyOverall)
  # resBest <- rev(resBest)
  ...
  for (i in 1:length(resultRS1)) {
    ...
    if (brOrig > 1) {
      ColOri <- rbPal1(brOrig)[as.numeric(cut(as.numeric(bestrestGA$AbschGesamt),breaks = brOrig))]
    } else {
      ColOri <- "green"
    }
    ...
    raster::plot(Polygon1, col=col2res, main=paste("Original - Best Energy:", (best+1)-i, "\n","Energy Output",
                                                   bestrestGA$EnergyOverall[[1]],"kW", "\n", "Efficiency:",
                                                   bestrestGA$EfficAllDir[[1]]));
    raster::plot(Grid,add=T)
    graphics::mtext("Total Wake Effect in %", side = 2)
    graphics::points(bestrestGA$X,bestrestGA$Y,cex=2,pch=20,col=ColOri)
    graphics::text(bestrestGA$X, bestrestGA$Y, round(bestrestGA$AbschGesamt,0), cex=0.8, pos=1, col="black")
    distpo <- stats::dist(x = cbind(bestrestGA$X,bestrestGA$Y),method = "euclidian")
    ...
    raster::plot(Polygon1, col=col2res, main=paste("Random Search - Best Energy:", (best+1)-i, "\n","Energy Output",
